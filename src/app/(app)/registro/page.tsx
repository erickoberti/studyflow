...
export default async function RegistroPage({ searchParams }: { searchParams?: { novo?: string } }) {
  ...
  const [current, previews, recentSessions, settings, active, dashboard, focusPreview] = await Promise.all([
  ...
  const next = focusPreview[1];
  return <div className="space-y-5 pb-20 lg:pb-0"><header className="flex items-center justify-between"><div><h1 className="text-3xl font-black text-slate-900 dark:text-white">Estudar</h1><p className="mt-1 text-slate-500">Uma sessão por vez. O resto pode esperar.</p></div><Link href="/registros" className="rounded-xl border border-primary/30 bg-primary/10 px-4 py-2.5 text-sm font-bold text-primary">Ver sessões</Link></header><ActiveStudyPanel initialActive={active as never} suggestion={current as never} nextSuggestion={next ? { discipline: next.discipline.name, subject: next.subject.name } : null} summary={{ todayMinutes: todayData ? dashboard.totals.totalEstimatedMinutes : 0, todayQuestions: todayData?.questions ?? 0, dailyGoal: dashboard.totals.dailyQuestionsGoal, streak: dashboard.totals.streakDays }} />
