...
export default async function RegistroPage({ searchParams }: { searchParams?: { novo?: string } }) {
  ...
  return <div className="space-y-5 pb-20 lg:pb-0">
    ...
    <ActiveStudyPanel userId={user.id} studyGuideId={guide.id} initialActive={active as never} suggestion={current as never} nextSuggestion={next ? { discipline: next.discipline.name, subject: next.subject.name } : null} summary={{ todayMinutes: todayData ? dashboard.totals.totalEstimatedMinutes : 0, todayQuestions: todayData?.questions ?? 0, dailyGoal: dashboard.totals.dailyQuestionsGoal, streak: dashboard.totals.streakDays }} />
