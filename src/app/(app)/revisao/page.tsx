...
export default async function RevisaoPage() {
  ...
  const data = await getReviewSuggestions(user.id, guide.id);
  ...
  return (
    <div className="space-y-8 pb-16 lg:pb-0">
      <header>
        <h1 className="text-4xl font-black text-slate-900 dark:text-white">Painel de Revisao</h1>
        <p className="mt-1 text-slate-500 dark:text-slate-400">Otimize seu aprendizado focando nos topicos mais criticos.</p>
      </header>

      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_420px]">
        <div className="space-y-6">
          <article className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-primary/20 dark:bg-[#161126]">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-primary">Sugestao de prioridade</p>
            {data.weak[0] ? (
              <div className="mt-3 grid grid-cols-1 gap-4 md:grid-cols-[220px_1fr]">
                <div className="rounded-xl bg-gradient-to-br from-primary to-[#6d38e0] p-4 text-white">
                  <p className="text-xs font-bold uppercase">Alta urgencia</p>
                  <p className="mt-6 text-4xl font-black">S</p>
                </div>
                <div>
                  ...
                  <p className="mt-2 text-slate-600 dark:text-slate-400">Voce esta com baixo desempenho e sem revisao recente nesse topico.</p>
                  <div className="mt-4 flex items-center justify-between">
                    <p className="text-sm text-slate-500">Ultimo estudo: {when(data.weak[0].lastStudy)}</p>
                    <button className="rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white">Iniciar agora</button>
                  </div>
                </div>
              </div>
            ) : (
              <p className="mt-2 text-slate-500">Sem dados de revisao ainda.</p>
            )}
          </article>

          <section>
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Materias pendentes</h3>
            </div>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {data.stale.slice(0, 4).map((item) => (
                <article key={item.id} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-primary/20 dark:bg-[#161126]">
                  ...
                  <p className="mt-2 text-xs text-slate-500">Ultimo estudo: {when(item.lastStudy)}</p>
                </article>
              ))}
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 gap-4 md:grid-cols-4">
            <article className="rounded-xl border border-slate-200 bg-white p-4 dark:border-primary/20 dark:bg-[#161126]">
              <p className="text-xs uppercase tracking-wider text-slate-500">Media semanal</p>
            ...
            <article className="rounded-xl border border-slate-200 bg-white p-4 dark:border-primary/20 dark:bg-[#161126]">
              <p className="text-xs uppercase tracking-wider text-slate-500">Cards de revisao</p>
        ...
        <aside className="rounded-2xl border border-red-300 bg-red-50 p-6 dark:border-red-500/20 dark:bg-[#0d1a33]">
          ...
          <p className="mt-1 text-slate-600 dark:text-slate-400">Topicos com desempenho abaixo de 60% recentemente.</p>
          ...
          <button className="mt-6 w-full rounded-xl border border-primary/40 bg-primary/10 py-3 text-sm font-bold text-primary">Reforcar topicos criticos</button>
          <p className="mt-4 text-xs text-slate-500">Usuario: {user.name ?? "Aluno"}</p>
        </aside>
      </section>
    </div>
  );
