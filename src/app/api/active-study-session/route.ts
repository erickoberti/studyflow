...
const commandSchema = z.object({ command: z.enum(["start", "pause", "resume", "cancel", "finish"]), operationId: z.string().min(1).optional(), id: z.string().optional(), version: z.number().int().optional(), mode: z.enum(["CYCLE", "AVULSO"]).optional(), disciplineId: z.string().optional(), subjectId: z.string().optional(), timerRunning: z.boolean().optional(), questions: z.number().int().min(0).optional(), correct: z.number().int().min(0).optional(), minutes: z.number().int().min(1).optional(), activityType: z.enum(["QUESTIONS", "CLASS", "READING", "PDF_READING", "REVIEW"]).optional(), advanceCycle: z.boolean().optional(), notes: z.string().max(4000).optional() });
