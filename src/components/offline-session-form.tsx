...
import { FileText, ListChecks, RefreshCcw, Scale, Video } from "lucide-react";
...
export function OfflineSessionForm() {
  ...
  const [mode, setMode] = useState<OfflineStudyMode | "GENERAL">("CYCLE");
  ...
  async function submit() {
    ...
    if ((activityType === "QUESTIONS" || mode === "GENERAL") && correct + wrong <= 0) return toast.error("Informe ao menos um acerto ou erro.");
    setBusy(true);
    try {
      if (mode === "GENERAL") {
        ...
        await createStandaloneOfflineSession({ userId, studyGuideId, mode: "AVULSO", scope: "GENERAL", disciplineId: null, subjectId: null, cycleEntryId: null, disciplineName: "Todas as matérias", subjectName: "Revisão geral", startedAt: sessionDate.toISOString(), pausedAt: null, finishedAt: new Date().toISOString(), accumulatedSeconds: minutes * 60, questions: correct + wrong, correct, wrong, activityType: "REVIEW", advanceCycle: false, difficulty, notes: notes.trim() || null, date: sessionDate.toISOString() });
      } else if (mode === "AVULSO") {
      ...
      toast.success(mode === "CYCLE" ? advanceCycle ? "Sessão salva. O ciclo avançará após sincronizar." : "Sessão salva; você continuará no mesmo assunto." : mode === "GENERAL" ? "Revisão geral salva para sincronização." : "Estudo avulso salvo para sincronização.");
      const now = new Date(); setCorrect(0); setWrong(0); setActivityType(mode === "GENERAL" ? "REVIEW" : "QUESTIONS"); setAdvanceCycle(true); setNotes(""); setDifficulty("Média"); setMinutes(60); setDate(dayKey(now)); setTime(timeKey(now)); setDisciplineId(""); setSubjectId("");
  ...
  return <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_320px]">
    <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
      ...
      <div className="mt-6 grid grid-cols-3 gap-2 rounded-2xl bg-slate-100 p-1 dark:bg-slate-900">{(["CYCLE", "AVULSO", "GENERAL"] as const).map((value) => <button key={value} type="button" onClick={() => { setMode(value); setActivityType(value === "GENERAL" ? "REVIEW" : "QUESTIONS"); }} className={`min-h-11 rounded-xl px-2 text-sm font-black ${mode === value ? "bg-white text-primary shadow dark:bg-slate-800" : "text-slate-500"}`}>{value === "CYCLE" ? "Modo ciclo" : value === "AVULSO" ? "Por matéria" : "Revisão geral"}</button>)}</div>
      {mode !== "GENERAL" ? <div className="mt-4"><p className="text-sm font-bold">O que você estudou?</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4"><button type="button" onClick={() => setActivityType("QUESTIONS")} className={`inline-flex min-h-16 flex-col items-center justify-center gap-1 rounded-xl border text-sm font-black ${activityType === "QUESTIONS" ? "border-primary bg-primary/10 text-primary" : "border-slate-200 dark:border-slate-700"}`}><ListChecks size={18} /> Questões</button><button type="button" onClick={() => { setActivityType("CLASS"); setCorrect(0); setWrong(0); }} className={`inline-flex min-h-16 flex-col items-center justify-center gap-1 rounded-xl border text-sm font-black ${activityType === "CLASS" ? "border-primary bg-primary/10 text-primary" : "border-slate-200 dark:border-slate-700"}`}><Video size={18} /> Videoaula</button><button type="button" onClick={() => { setActivityType("READING"); setCorrect(0); setWrong(0); }} className={`inline-flex min-h-16 flex-col items-center justify-center gap-1 rounded-xl border text-sm font-black ${activityType === "READING" ? "border-primary bg-primary/10 text-primary" : "border-slate-200 dark:border-slate-700"}`}><Scale size={18} /> Lei seca</button><button type="button" onClick={() => { setActivityType("PDF_READING"); setCorrect(0); setWrong(0); }} className={`inline-flex min-h-16 flex-col items-center justify-center gap-1 rounded-xl border text-sm font-black ${activityType === "PDF_READING" ? "border-primary bg-primary/10 text-primary" : "border-slate-200 dark:border-slate-700"}`}><FileText size={18} /> PDF/material</button></div></div> : <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/5 p-4"><p className="flex items-center gap-2 font-black text-primary"><RefreshCcw size={18} /> Revisão de todas as matérias</p><p className="mt-1 text-sm text-slate-500">Conta questões e tempo, sem alterar o ciclo ou uma matéria específica.</p></div>}
      {mode === "CYCLE" ? <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/5 p-4"><p className="text-xs font-black uppercase text-primary">Posição atual</p><p className="mt-2 font-black">{suggestion?.subject.discipline.name ?? "Sem sugestão disponível"}</p><p className="text-sm text-slate-500">{suggestion?.subject.name ?? "Conecte-se para atualizar o ciclo."}</p></div> : mode === "AVULSO" ? <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-bold">Disciplina<select aria-label="Disciplina" value={disciplineId} onChange={(event) => { setDisciplineId(event.target.value); setSubjectId(""); }} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 dark:border-slate-700 dark:bg-slate-900"><option value="">Selecione uma disciplina</option>{disciplines.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-sm font-bold">Assunto<select aria-label="Assunto" disabled={!disciplineId || subjects.length === 0} value={subjectId} onChange={(event) => setSubjectId(event.target.value)} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 disabled:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:disabled:bg-slate-800"><option value="">{disciplineId ? subjects.length ? "Selecione um assunto" : "Nenhum assunto ativo" : "Escolha uma disciplina primeiro"}</option>{subjects.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div> : null}
      <div className="mt-5 grid gap-4 sm:grid-cols-3"><label className="text-sm font-bold">Data<input type="date" max={dayKey(new Date())} value={date} onChange={(event) => setDate(event.target.value)} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 dark:border-slate-700 dark:bg-slate-900" /></label>{mode !== "CYCLE" ? <label className="text-sm font-bold">Horário<input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 dark:border-slate-700 dark:bg-slate-900" /></label> : null}{activityType === "QUESTIONS" || mode === "GENERAL" ? <><label className="text-sm font-bold">Acertos<input type="number" min={0} value={correct} onChange={(event) => setCorrect(Math.max(0, Number(event.target.value) || 0))} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 dark:border-slate-700 dark:bg-slate-900" /></label><label className="text-sm font-bold">Erros<input type="number" min={0} value={wrong} onChange={(event) => setWrong(Math.max(0, Number(event.target.value) || 0))} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 dark:border-slate-700 dark:bg-slate-900" /></label></> : null}<label className="text-sm font-bold">Tempo (min)<input type="number" min={1} value={minutes} onChange={(event) => setMinutes(Math.max(1, Number(event.target.value) || 1))} className="mt-1.5 h-12 w-full rounded-xl border bg-white px-3 dark:border-slate-700 dark:bg-slate-900" /></label></div>
      ...
      {activityType === "QUESTIONS" || mode === "GENERAL" ? <div className="mt-4 rounded-xl bg-slate-100 p-3 text-sm dark:bg-slate-900">Total: <b>{total}</b> · Aproveitamento: <b>{total ? Math.round(correct / total * 100) : 0}%</b></div> : <div className="mt-4 rounded-xl bg-primary/5 p-3 text-sm font-semibold">{STUDY_ACTIVITY_LABELS[activityType]} contará o tempo e a sessão normalmente.</div>}
      <button type="button" disabled={busy || ((activityType === "QUESTIONS" || mode === "GENERAL") && total === 0)} onClick={submit} className="mt-5 min-h-12 rounded-xl bg-primary px-5 font-black text-white disabled:opacity-50">{busy ? "Salvando..." : mode === "GENERAL" ? "Salvar revisão geral localmente" : `Salvar ${STUDY_ACTIVITY_LABELS[activityType].toLocaleLowerCase("pt-BR")} localmente`}</button>
    </article>
