...
export function OfflineSessionForm() {
  ...
  const [activityType, setActivityType] = useState<"QUESTIONS" | "CLASS">("QUESTIONS");
  ...
  async function submit() {
    ...
    try {
      if (mode === "AVULSO") {
        ...
        await queueOfflineSessionOperation({ userId, studyGuideId, type: "FINISH_SESSION", session: { ...started.session, accumulatedSeconds: minutes * 60, questions: activityType === "CLASS" ? 0 : correct + wrong, correct: activityType === "CLASS" ? 0 : correct, wrong: activityType === "CLASS" ? 0 : wrong, activityType, advanceCycle, difficulty, notes: notes.trim() || null, date: new Date(`${date}T12:00:00-03:00`).toISOString() } });
      }
  ...
  return <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1fr_320px]">
    <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
      ...
      <div className="mt-4"><p className="text-sm font-bold">O que você estudou?</p><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={() => setActivityType("QUESTIONS")} className={`inline-flex min-h-14 items-center justify-center gap-2 rounded-xl border font-black ${activityType === "QUESTIONS" ? "border-primary bg-primary/10 text-primary" : "border-slate-200 dark:border-slate-700"}`}><ListChecks size={18} /> Questões</button><button type="button" onClick={() => { setActivityType("CLASS"); setCorrect(0); setWrong(0); }} className={`inline-flex min-h-14 items-center justify-center gap-2 rounded-xl border font-black ${activityType === "CLASS" ? "border-primary bg-primary/10 text-primary" : "border-slate-200 dark:border-slate-700"}`}><BookOpenCheck size={18} /> Aula</button></div></div>
      ...
      <button type="button" disabled={busy || (activityType === "QUESTIONS" && total === 0)} onClick={submit} className="mt-5 min-h-12 rounded-xl bg-primary px-5 font-black text-white disabled:opacity-50">{busy ? "Salvando..." : activityType === "CLASS" ? "Salvar aula localmente" : "Salvar localmente"}</button>
    </article>
