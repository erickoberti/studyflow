...
export function canonicalSessionOperationPayload(input: {
  type: string; mode?: string; scope?: string; disciplineId?: string | null; subjectId?: string | null; sessionId?: string | null;
  ...
  if (input.type === "CREATE_STANDALONE_SESSION") return { mode: "AVULSO", ...(input.scope === "GENERAL" ? { scope: "GENERAL" } : {}), disciplineId: input.disciplineId, subjectId: input.subjectId, questions: input.questions, correct: input.correct, minutes: input.minutes, notes: input.notes, date: input.date };
